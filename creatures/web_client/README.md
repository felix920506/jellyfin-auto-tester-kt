...
action at a time from live page evidence, and sends the final standard
`ExecutionResult` unchanged to `execution_done`. The tool owns one active
session at a time; actions and finalize apply to the current session without an
explicit identifier.
