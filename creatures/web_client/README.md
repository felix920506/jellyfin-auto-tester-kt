...
cursor with `advance_step`, and sends a compact final payload to
`execution_done`. The full `ExecutionResult` remains in artifacts. The tool owns
one active session and one current plan step at a time; actions, step
advancement, and finalize apply to the current session without an explicit
identifier.
