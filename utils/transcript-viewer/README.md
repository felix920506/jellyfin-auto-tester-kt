...
4.  The app will automatically find and list all `transcript.json` files and
    load adjacent `transcript_metadata.json` files when present.
...
- **Split Metadata**: Shows run details from `transcript_metadata.json` while
  keeping `transcript.json` focused on model messages.
